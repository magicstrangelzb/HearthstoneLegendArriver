...
**右上角实时日志浮窗**（品牌行 + 状态面板 + 延时进度条；告警行红色高亮）
...
自动对局开始时，屏幕**右上角**会出现一个**置顶半透明**小窗（292×640），实时滚动显示自动化日志与当前状态：
...
- **告警行红色加粗**：`⚠️ 炉石已退出…`、`⚠️ 用户 ID 与日志玩家名不匹配…` 等；
- `[推荐]` / `[执行]` → 白，`等待` → 灰；
- 日志**增量追加**，缓冲 **2000 行**，可在底部停留时自动跟随；
- 长日志**自动换行**；底部有**延时进度条 + 文字说明**（活人感的随机延时也显示在这里）；
- 浮窗按钮：`▶ 开始对战`、`⏹ 中止 / ▶ 恢复`（恢复**不清零**已累计战绩）、`⏸ 本局结束后停止`、`💾 保存日志`（写入 `logs/`）、`🚪 退出脚本`；
